...
void TcpConnection::sendInloop(const void *message,size_t len)
{
    ...
    if(state_ == Kdisconnected)//连接断开 无法发送
    {
        //LOG_ERROR("connect has disconnected cannot send");
    }
...
void TcpConnection::send(const std::string &buf){
    if(state_ == Kconnecting)//如果连接还在  就发送数据
    {
        /*
    ...
    {   
        ...
        if(n >0)
        {
            ...
            if(outputbuffer_.readableBytes() == 0)//发送完成
            {
                ...
                if(state_ == Kdisconnecting)//正在关闭 服务器  用户执行了TcpConnection::shutdown函数
                {
...
{
    if(state_ == Kconnected)//连接已经建立了 才可以关闭
    {
...
{
    if(state_ == Kconnected)//判断状态是否是 已连接
    {
